...
def interface(name_of_interface: str, enabled: int | None):
    ...
    if enabled not in (1, 0, None):
        raise ValueError(
            "values of parameter enabled must be either 1 or 0 integers or None"
        )
    ...
    if p == "Windows":
        if isinstance(enabled, int):
            cmd = "netsh interface set interface {} {}".format(
                name_of_interface, "admin=ENABLED" if enabled else "admin=DISABLED"
            )
        ...
        if isinstance(enabled, int):
            cmd = "ifconfig {} {}".format(
                name_of_interface, "up" if enabled else "down"
            )
    ...
    if r.returncode == 0:
        if enabled in (1, 0):
            print(
                "'{}{}{}' is {}".format(
                    Fore.GREEN if enabled else Fore.RED,
                    name_of_interface,
                    Fore.RESET,
...
if __name__ == "__main__":
    ...
    if args.restart:
        ...
        r1 = interface(args.interface, 0)
        r2 = interface(args.interface, 1)

        print()
        exit(r1 or r2)

    elif (
        args.enable_disable is not None
        and not isinstance(args.enable_disable, int)
        or isinstance(args.enable_disable, int)
        and args.enable_disable > 1
    ):
